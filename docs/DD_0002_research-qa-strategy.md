...
**Data vs conclusions.** A finding's data matters even if the source's conclusion is wrong. A speaker who claims "Christianity was a mystery cult void of Judaism" may draw a wrong conclusion, but the mystery cult parallels they cite — initiation rites, tiered membership, ritual meals, secret teachings — are valuable data that the book can use with its own framing. The evaluator extracts the data; the author decides the conclusion. Never SKIP a finding because the source draws a wrong conclusion from good evidence.
