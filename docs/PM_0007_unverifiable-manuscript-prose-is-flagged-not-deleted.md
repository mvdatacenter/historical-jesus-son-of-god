...
- [x] [prevent] Added the enforcing diff check to the same section of `docs/REVIEW.md`: every `-` line removing prose from `chapter*.tex`, `preface.tex`, or `epilogue.tex` is enumerated in the PR body with a per-passage reason, verified by `git diff -- '*.tex' | grep '^-'`. This is the mechanical catch for the concealment shown in the evidence fence above, where a net-positive diffstat hid the loss of the section's thesis.
