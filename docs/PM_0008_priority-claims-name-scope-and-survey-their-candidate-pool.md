# PM-0008: Priority claims name their scope and survey their candidate pool

## What Happened

On 2026-08-20, pull request #173 (branch `feat/172-preserved-lyons-and-vienne-manuscript-branch-has`) added a paragraph to `chapter5.tex` opening with a priority claim about the churches of Lyons and Vienne, and the pull-request summary prepared for it described the paragraph as supplying "the moment the West enters from the inside". The sentence states no scope — first attested where, among what candidates — and the paragraph's own citations contain the countercase: the same witnesses write to Eleutherus, bishop of Rome, an older Western church standing in the very evidence cited. Ten lines above it, the chapter already dates Carthage's church to c. 180, three years after the paragraph's 177.
...
The same day, on the operator's order, the manuscript was swept for the same claim shape: priority and uniqueness claims — "first", "earliest", "oldest", "only", "no other", "not one", "never", "none" — published without a scope statement and without a survey of the candidate pool the claim quantifies over. The sweep confirmed twenty instances: the PR #173 trigger plus nineteen in the merged manuscript, every one of which predates the trigger and entered through earlier pull requests (the git blame summaries for the surveyed lines name PRs #13, #14, #15, #25, #26, #30, #33, #35, #54, #60, #62, and #99). Of the twenty: five are contradicted from the book's own pages or from sources already in this repository; three are contradicted by the corpus's own text at Acts 28:12–14; eight state a universal against a named standard countercase that has not been run through the citation pipeline; four state a universal over a pool the page never enumerates. A claim in the last two groups may yet survive its survey — what is recorded here is that it was published without one.
