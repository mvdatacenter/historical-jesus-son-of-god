...
- **Modern source labels stay in citations; source evidence stays in prose.** Bible verse numbers are acceptable because readers expect that convention, and manuscript shelfmarks may be necessary when identifying a witness. Modern editorial labels for ancient corpora, such as "Book of the Dead Spell 17," usually move to the citation while the prose preserves the source's concrete evidence. When revising label-heavy prose, compare before and after: short quotations such as "I know the name of that god" stay when they are accurate and relevant. A passing review keeps concrete quotations in the prose instead of trading them for generic paraphrase.
...
- **Chapter 1 stays at survey register.** Chapter 1 surveys scholarly models; it does not deep-dive into specific scholars' arguments. Any addition to ch1 is one overview-appropriate sentence, not a multi-sentence argument summary.
