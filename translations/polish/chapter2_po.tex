...
\section{Krolewska linia przez jego matke Marie}\label{sec:royal-lineage-through-his-mother-mary}
...
\section{Matka Jezusa byla Maria Chrystus, ostatnia prawowita dziedziczka dynastii Asmoneuszy.}\label{sec:jesuss-mother-was-mary-christ-the-last-rightful-heiress-to-the-hasmonean-dynasty.}
...
\section{Ucieczka Jezusa do Egiptu moze byc faktem historycznym, poniewaz rodzina asmonejska miala bardzo bliskie zwiazki z Egiptem.}\label{sec:jesus-fleeing-to-egypt-can-be-a-historical-fact-as-the-hasmonean-family-had-very-close-ties-to-egypt.}
...
\section{Ojciec Jezusa zostal zabity przez Heroda Wielkiego}\label{par:jesuss-father-was-killed-by-herod-the-great}
...
\section{Scena z Magami zachowuje realny protokol dworski Wschodu, a nie folklor.}\label{sec:magi-court-protocol}
...
\section{Kazde kolano zegnie sie przed Chrystusem.}\label{sec:every-knee-shall-bow-to-christ.}
...
Poprawne obliczenie zaczyna się od ustalenia, jak w praktyce powstawały groby ossuariowe.
Przechodzimy przez filtry, które ukształtowały próbę archeologiczną.
Każdy filtr zawęża możliwe dopasowania.
Ich łączny efekt wyznacza prawdopodobieństwo zbieżności.

Filtr 1 to próba elitarna.
Tylko rodziny wysokiego statusu używały ossuariów.
Tylko niektórym osobom w obrębie tych rodzin dawano ossuaria z inskrypcją.
Właściwe dane częstości pochodzą z tego ograniczonego zbioru, a nie z ogólnej populacji Judei.
To natychmiast unieważnia argument, że „Maria i Józef to częste imiona”, ponieważ korpus ossuariów nie jest próbą z szerokiej demografii wsi, lecz z małej, piśmiennej, miejskiej grupy elitarnej.

Filtr 2 to pięcioimienny klaster w grobie Talpiot.
Siedem ossuariów ma inskrypcje.
Pięć z tych inskrypcji zawiera imiona: Jezus, Józef, Maria, Jakub oraz grecką formę Mariamne.
...
Prawdopodobieństwo, że losowa grupa siedmiu elitarnych pochówków zawiera te pięć konkretnych imion, jest iloczynem tych częstości pomnożonym przez 21 kombinacji wyboru pięciu z siedmiu.
Daje to w przybliżeniu $2 \times 10^{-4}$ dla pojedynczego grobu.
Przy około dwustu opisanych grobach rodzinnych w Jerozolimie surowa oczekiwana liczba takich klastrów wynosi zaledwie kilka setnych, co już stawia Talpiot na granicy rzadkości, nawet zanim uwzględnimy relacje.

Filtr 3 to zgodność ojcowska.
Inskrypcje z Talpiot obejmują Jezusa, syna Józefa, oraz Jakuba, syna Józefa.
Ta konfiguracja spina trzy rzadkie męskie imiona w jednej linii ojcowskiej.
Samo wylosowanie Jezusa, Józefa i Jakuba przy ich częstościach jest już mało prawdopodobne; wymóg, by dwóch z nich miało tego samego ojca, zmniejsza przypadkową zbieżność co najmniej o rząd wielkości.
Powiązanie braterskie Jezusa i Jakuba jest już implikowane przez wspólną inskrypcję ojcowską i nie musi być liczone podwójnie.

Filtr 4 to rzadkość formuły inskrypcji z ossuarium Jakuba.
Wśród ponad tysiąca znanych inskrypcji ossuariów tylko jedna używa konstrukcji ``brat''.
Ta inskrypcja identyfikuje brata jako Jezusa.
Ta rzadkość działa jak filtr jeden na tysiąc.
Formuła ma znaczenie kulturowe: inskrypcja typu „brat” pojawia się tylko wtedy, gdy tożsamość brata niesie wyjątkową wagę.
Ta cecha łączy się bezpośrednio z klastrem Talpiot i drastycznie zmniejsza prawdopodobieństwo przypadkowego dopasowania.

Filtr 5 to ograniczenie fizyczne wynikające z analizy patyny.
Testy geochemiczne pokazują, że patyna ossuarium Jakuba pasuje do patyny grobu w Talpiot.
To ustanawia łączność pozastatystyczną: jeśli ossuarium Jakuba pochodziło z grobu Talpiot, to grób zawierał wszystkie pięć kluczowych imion, właściwą strukturę ojcowską oraz jedyną znaną w korpusie inskrypcję typu ``brat''.
Tego zestawu nie da się odtworzyć samymi częstościami imion; jest on zakotwiczony w dowodzie fizycznym.
