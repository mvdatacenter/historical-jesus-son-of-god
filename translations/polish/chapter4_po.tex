...
W Ewangelii Marii, Ewangelii Tomasza i \textit{Pistis Sophia} to zawsze Piotr atakuje Marię, ponieważ jest kobietą i ponieważ otrzymała szczególne objawienie \cite{deboer:magdalene}.
...
Współczesne badania krytyczne jednoznacznie wykazały, że maskulinizacja była ideologicznym zniekształceniem.
...
Analiza rękopisów \cite{schrader:martha} pokazuje znaczną niestabilność tekstową w fragmentach o Marii i Marcie w J 11--12.
Dane są uderzające:
...
Tezę, że Ewangelia Jana szeroko czerpie z doktryny Logosu judaizmu hellenistycznego, którego najjaśniejszym świadkiem jest Filon, sformułowano wcześnie i nigdy jej nie obalono \cite{dodd:fourthgospel}.
Jana i Filona określono jako „dwie wariacje na temat jednego Logosu”: dzielą ten sam schemat pojęciowy, przy czym wyjątkową nutą Jana jest wcielenie \cite{attridge:essays}.
Obaj w podobny sposób odczytują tradycje Wyjścia --- jak manna i Namiot Spotkania --- przez pryzmat Logosu, co czyni ich zbieżność zbyt specyficzną, by była przypadkiem \cite{borgen:philo}.
Odczytania ostrożniejsze przyznają, że Logos Filona stanowi jedno z tła, obok żydowskich tradycji \emph{Memra} i Mądrości \cite{brown:johannine}.
Udokumentowano również, jak późniejsi chrześcijanie swobodnie sięgali po Filona, z zastrzeżeniem, że bezpośredniej zależności Jana od Filona nie da się udowodnić ponad wszelką wątpliwość \cite{runia:philo}.
Ujęcie zorientowane na judaizm odczytuje Prolog Jana jako midrasz do „Memry” w ramach binitarnego monoteizmu judaizmu drugiej świątyni, z Filonem lub bez niego \cite{boyarin:jewishgospels}.
...
Najmocniejszy wniosek idzie dalej: filonowa teologia Logosu jest sama w sobie chrystologiczna i antycypuje kategorie, które Jan później zastosuje do Jezusa \cite{hillar:logos}.
...
Jednak alternatywa --- sformułowana najpierw w \cite{harnack:marcion}, a rozwijana przez późniejsze analizy tekstualne --- głosi, że Ewangelia Marcjona odzwierciedla wcześniejszą redakcję Łukasza.
