...
% ----- Bibliography -----
% Same references.bib as the English edition, at the repository root; the CI
% Polish build runs latexmk from this directory, so the path is relative to it.
\usepackage[
  backend=biber,
  style=authortitle,
  sorting=nyt,
...
\backmatter
\nocite{*}
\printbibliography[keyword=ancient,title={Starożytne źródła pierwotne}]
\printbibliography[keyword=patristic,title={Ojcowie Kościoła i wczesna literatura chrześcijańska}]
\printbibliography[keyword=modern,title={Opracowania współczesne}]
